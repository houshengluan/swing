...
\begin{itemize}
\item int (整数), 如 1, 只有一种整数类型 int，表示为长整型，没有 python2 中的 Long。以0x开头的是十六进制，以0开头的是八进制。
\item bool (布尔), 如 True。
\item float (浮点数), 如 1.23、3E-2
\item complex (复数), 如 1 + 2j、 1.1 + 2.2j，还可以用complex(a, b)表示，其中a是实部，b是虚部，二者都是float类型
\end{itemize}
...
\item 减 -
\item 乘 *
\item 除 /(返回float类型) //(返回下取整)。
数值的除法包含两个运算符：/ 返回一个浮点数，// 得到的并一定是整数类型的数，它与分母分子的数据类型有关系。
\item 幂 **
\item 求余数 \%
\end{itemize}
...
\subsubsection{成员运算}
\begin{itemize}
\item in, x in y表示如果x在y序列中，返回True
\item not in
\end{itemize}

\subsubsection{身份运算}
\begin{itemize}
\item is 判断两个标识符是不是引用自一个对象
\item is not 判断两个标识符是不是引用自不同对象	
\end{itemize}
...
指数 $>$ 按位非、正号、负号 $>$ 乘、除、取模\% $>$ 加减 $>$ 左右移位 $>$ 按位与 $>$ 按位或、按位异或 $>$ 大于、大于等于、小于、小于等于 $>$ == != $>$ 赋值运算 $>$ 身份运算 $>$ 成员运算 $>$ 逻辑运算
